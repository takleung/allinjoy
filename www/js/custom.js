...
function init(){
    ...
    */
    ...
    function dual_sidebar(){
        $('#openLeftSideBar').show();
        dualSnapper = new Snap({
        ...
        $('.close-sidebar').click(function(){dualSnapper.close(); return false;});
        $('.open-left-sidebar').click(function() {
            //$(this).toggleClass('remove-sidebar');
            if( dualSnapper.state().state=="left" ){
                dualSnapper.close();
            } else {
                dualSnapper.open('left');
            }
            return false;
        }); 
        $('.open-right-sidebar').click(function() {
            //$(this).toggleClass('remove-sidebar');
            if( dualSnapper.state().state=="right" ){
                dualSnapper.close();
            } else {
                dualSnapper.open('right');
            }
            return false;
        });
        dualSnapper.on('open', function(){$('.back-to-top-badge').removeClass('back-to-top-badge-visible');});
        dualSnapper.on('drag', function(){$('.back-to-top-badge').removeClass('back-to-top-badge-visible');});
    };    
    
    function left_sidebar(){
        var leftSnapper = new Snap({
            element: document.getElementById('content'),
            elementMirror: document.getElementById('header-fixed'),
            elementMirror2: document.getElementById('footer-fixed'),
            disable: 'right',
        ...
        $('.close-sidebar').click(function(){leftSnapper.close(); return false;});
        $('.open-left-sidebar').click(function() {
            //$(this).toggleClass('remove-sidebar');
            if( leftSnapper.state().state=="left" ){
                leftSnapper.close();
            } else {
                leftSnapper.open('left');
            }
            return false;
        }); 
        leftSnapper.on('open', function(){$('.back-to-top-badge').removeClass('back-to-top-badge-visible');});
        leftSnapper.on('drag', function(){$('.back-to-top-badge').removeClass('back-to-top-badge-visible');});
    };    
...
function mainInit(){
    ...
    changeLang(lang, function(langPack){
        $('body').addClass("dual-sidebar");
        $('body').removeClass("no_sidebar");
        $("#header_logo").hide();

        // $("#sideBarLeftMain").show();
        // $("#snapDrawers").show();
        app.initialize();
        contentSwaper("HOME");
        getLeftSideBar();
...
function getLeftSideBar(){
    // $("#leftSideBarREsponsivePanel").load("template/side_menu.html");
      $.get('template/side_menu.html', function(templateData){
       var result =  $("#leftSideBarREsponsivePanel").html(_.template(templateData,{langPack:langPack}));
...
function getSponsor(){
          ...
          success: function(msg) {
              ...
              htmlString+='<div style="width:70%;margin-left: auto;margin-right: auto;">';
              htmlString+='<a href="'+result.logo_url+'" target="_blank">';
              htmlString+='<img class="preload-image responsive-image" alt="img" src="http://2017.marathon.clinic/wp-content/uploads/'+result.logo_path+'" style="display: block;">';
              htmlString+='</a>'
              htmlString+='<p style="text-align: center; margin-bottom:20px;">'+result.logo_name+'</p>';
              htmlString+='</div>';

            });
              $('#sponsorResponsivePanel').empty();
              $('#sponsorResponsivePanel').html(htmlString);
...
function getHomePic(){
          $.ajax({
          type: 'POST',
          url: contextRoot+'/mobile_app_photo.php',
          data: { 
            mode :lang
          },
          success: function(msg) {
              ...
              $('#homeTopPic').attr('src', imageContextRoot+query.index_slide_mobile);
              $('#homeRunningProfilePic').attr('src', imageContextRoot+query.mobile_app_photo_1);
              $('#homeFitnessProfilePic').attr('src', imageContextRoot+query.mobile_app_photo_2);
              $('#homeTranningPlanPic').attr('src', imageContextRoot+query.mobile_app_photo_3);
              $('#homeEnterLogPic').attr('src', imageContextRoot+query.mobile_app_photo_4);
              $('#homeViewLogPic').attr('src', imageContextRoot+query.mobile_app_photo_5);
...
function changeLang(lang, callback){
          $.ajax({
              type: 'GET',
              url: contextRoot+'/get_lang.php',
              data: { 
                mode :lang
              },
              success: function(msg) {
                  langPack = JSON.parse(msg);
                  callback(langPack);
...
function changeLangWithContentSwapper(){
        if (lang =="ENG"){
            lang = "CHI";
        }else{
            lang = "ENG";
          ...
          $.ajax({
              ...
              success: function(msg) {
                  ...
                  langPack = JSON.parse(msg);
                  contentSwaper(currentDestination);
                  getLeftSideBar();
